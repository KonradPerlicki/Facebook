...
<a href="{{ route('profile', $user->username) }}">  
    <div class="avatar relative rounded-md overflow-hidden w-full h-24 mb-2"> 
        <img src="{{ $user->prof_image }}" alt="" class="w-full h-full object-cover absolute">
    </div>
    <div class="text-sm truncate"> {{$user->full_name}} </div>
</a>
@else
<div class="card p-2">
    <a href="{{ route('profile', $user->username) }}">
        <img src="{{ $user->prof_image }}" class="h-36 object-cover rounded-md shadow-sm w-full">
    </a>
    <div class="pt-3 px-1">
        <a href="{{ route('profile', $user->username) }}" class="text-base font-semibold mb-0.5"> {{$user->full_name}} </a>
